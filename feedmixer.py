...
If any of the `feeds` URLs cannot be fetched or parsed, the `error_urls`
attribute will contain a dictionary whose keys are the URLs which generated an
error (if any did), and whose associated values are an Exception object which
contains a description of the error (and http status code if applicable).
